...
# خانة تخصيص البرومبت (تمت إضافتها هنا)
st.subheader("📝 تخصيص البرومبت (Prompt Customization)")
custom_prompt = st.text_area("البرومبت الموجه للذكاء الاصطناعي:", value=default_prompt, height=200)
    ...
    if not api_key:
        ...
        with zipfile.ZipFile(zip_buffer, "w") as zf:
            for file in uploaded_files:
                ...
                chunks, total_blocks = split_srt_into_blocks(srt_content, batch_size)
        ...
        zip_buffer.seek(0)
        st.download_button("📥 تحميل جميع الملفات (ZIP)", data=zip_buffer, file_name="Translated_Subtitles.zip", mime="application/zip")
        st.balloons()
